...
public class LexerRegressionTest {
  ...
  public void testEndOfFile() throws Exception {
    ...
    try (Lexer lexer = new Lexer(testReader);) {
      ...
      assertEquals(1, token.getLeftColumn());
      assertEquals(1, token.getRightColumn());
    }
  }
  ...
  public void testWhitespaceIsIgnored() throws Exception {
    ...
    try (Lexer lexer = new Lexer(testReader);) {
      ...
      assertEquals(TokenKind.EOF, token.getKind());
      assertEquals(4, token.getLeftColumn());
      assertEquals(4, token.getRightColumn());
    }
  }
  ...
  private static Stream<Arguments> provideBuiltInKeywords() {
    return Stream.of(Arguments.of(" program  ", TokenKind.Program, 1, 7),
        Arguments.of(" int  ", TokenKind.IntType, 1, 3),
        Arguments.of(" boolean  ", TokenKind.BooleanType, 1, 7),
        Arguments.of(" if  ", TokenKind.If, 1, 2), Arguments.of(" then  ", TokenKind.Then, 1, 4),
  ...
  private static Stream<Arguments> provideBuiltInOperators() {
    return Stream.of(Arguments.of(" + ", TokenKind.Plus, 1, 1),
        Arguments.of(" - ", TokenKind.Minus, 1, 1), Arguments.of(" / ", TokenKind.Divide, 1, 1),
        Arguments.of(" * ", TokenKind.Multiply, 1, 1), Arguments.of(" & ", TokenKind.And, 1, 1),
        Arguments.of(" | ", TokenKind.Or, 1, 1), Arguments.of(" = ", TokenKind.Assign, 1, 1),
        Arguments.of(" == ", TokenKind.Equal, 1, 2), Arguments.of(" != ", TokenKind.NotEqual, 1, 2),
        Arguments.of(" < ", TokenKind.Less, 1, 1), Arguments.of(" <= ", TokenKind.LessEqual, 1, 2));
  }

  private static Stream<Arguments> provideBuiltInSeparators() {
    return Stream.of(Arguments.of(" , ", TokenKind.Comma, 1, 1),
  ...
  private static Stream<Arguments> provideIdentifierExamples() {
    return Stream.of(Arguments.of(" _underscore ", TokenKind.Identifier, 1, 11),
  ...
  private static Stream<Arguments> provideIntegerExamples() {
    return Stream.of(Arguments.of(" 012345 ", TokenKind.IntLit, 1, 6),
